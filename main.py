...
def osr_screen():
    ...
    ocr_data = pytesseract.image_to_data(
        img, lang="eng", output_type="dict", config="--psm 11"
    )
...
def is_inside_specified_area(position):
    ...
    if specified_area:
        ...
        if (
            x < specified_area["left"]
            or x > specified_area["right"]
            or y < specified_area["top"]
            or y > specified_area["bottom"]
        ):
            return False
    return True
...
def find_on_screen(phrase):
    ...
    if previous_command == "right_click":
        verbose(
            "Previous command was right click, only looking below and to the right of the cursor"
        )
